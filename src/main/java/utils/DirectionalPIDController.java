...
public class DirectionalPIDController implements Sendable, AutoCloseable {
   ...
   */
  public DirectionalPIDController(double kp, double ki, double kd) {
    this(kp, ki, kd, 0.02);
  }
  ...
  public DirectionalPIDController(double kp, double ki, double kd, double period) {
    m_kp = kp;
    m_ki = ki;
    m_kd = kd;
   ...
   *
   * @param measurement The current measurement of the process variable.
   * @param hasDirection Whether the error should prioritize a given direction.
   * @param postiveDirection Whether the error will be positive or negative.
   * @return The next controller output.
   */
  private double calculate(double measurement, boolean hasDirection, boolean postiveDirection) {
    ...
    if (m_continuous) {
      if (hasDirection) {
        if (postiveDirection) {
            m_error = Math.abs(m_setpoint - m_measurement);
        } else {
            m_error = -Math.abs(m_setpoint - m_measurement);
        }
      } else {
